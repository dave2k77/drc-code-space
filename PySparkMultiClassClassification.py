##############################################################################
# COMPARING MULTICLASS CLASSIFICATION MODELS USING PYSPARK                   #
...
unsw_data = spark.read.csv(unsw_data_url, inferSchema=True).toDF("srcip", "sport", "dstip", "dsport", "proto", "state", "dur", "sbytes", "dbytes", "sttl", "dttl", "sloss", "dloss", "service", "sload", "dload", "spkts", "dpkts", "swin", "dwin", "stcpb", "dtcpb", "smeansz", "dmeansz", "trans_depth", "res_bdy_len", "sjit", "djit", "stime", "ltime", "sintpkt", "dintpkt", "tcprtt", "synack", "ackdat", "is_sm_ips_ports", "ct_state_ttl", "ct_flw_http_mthd", "is_ftp_login", "ct_ftp_cmd", "ct_srv_src", "ct_srv_dst", "ct_dst_ltm", "ct_src_ltm", "ct_src_dport_ltm", "ct_dst_sport_ltm", "ct_dst_src_ltm", "attack_cat", "label")

# FEATURE SELECTION AND VECTORISARION
from pyspark.ml.feature import StringIndexer, VectorAssembler

# CREATE A NEW DATAFRAME USING ONLY THE RELEVANT COLUMNS
unsw_net_data = unsw_data.select("state", "service", "sttl", "swin", "dwin", "ct_state_ttl", "ct_srv_src", "ct_srv_dst", "ct_dst_ltm", "ct_src_ltm", "ct_src_dport_ltm", "ct_dst_sport_ltm", "ct_dst_src_ltm", "tcprtt", "label", "attack_cat")

# INDEX THE CATEGORICAL FEATURE COLUMNS
indexed_unsw_net_data = StringIndexer(inputCols=["state", "service", "sttl", "swin", "dwin", "attack_cat"], outputCols=["state_index", "service_index", "sttl_index", "swin_index", "dwin_index", "attack_cat_index"]).fit(unsw_net_data).transform(unsw_net_data)

# REMOVE UNNECESSARY COLUMNS
indexed_unsw_net_data = indexed_unsw_net_data.drop("state", "service", "sttl", "swin", "dwin", "attack_cat")

# VECTORISE FEATURES COLUMNS
vectorised_unsw_net_data = VectorAssembler(inputCols=['ct_state_ttl', 'ct_srv_src', 'ct_srv_dst', 'ct_dst_ltm', 'ct_src_ltm', 'ct_src_dport_ltm', 'ct_dst_sport_ltm', 'ct_dst_src_ltm', 'tcprtt', 'state_index', 'service_index', 'sttl_index', 'swin_index', 'dwin_index'], outputCol='features').transform(indexed_unsw_net_data).select("features", "label", "attack_cat_index")
